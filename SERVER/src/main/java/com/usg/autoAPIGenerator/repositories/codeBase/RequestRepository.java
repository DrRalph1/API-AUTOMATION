package com.usg.autoAPIGenerator.repositories.codeBase;

import com.usg.autoAPIGenerator.entities.postgres.codeBase.RequestEntity;
...
@Repository("RequestRepositoryCodeBase")
public interface RequestRepository extends JpaRepository<RequestEntity, String> {
    ...
    @Query("SELECT r FROM RequestEntityCodeBase r WHERE r.collection.id = :collectionId AND r.folder IS NULL")
    ...
    @Query("SELECT r FROM RequestEntityCodeBase r WHERE :tag MEMBER OF r.tags")
    List<RequestEntity> findByTag(@Param("tag") String tag);

    @Query("SELECT r FROM RequestEntityCodeBase r WHERE " +
    ...
    @Query("SELECT r FROM RequestEntityCodeBase r WHERE r.implementationsCount > 0")
    List<RequestEntity> findRequestsWithImplementations();

    @Query("SELECT r FROM RequestEntityCodeBase r WHERE r.implementationsCount = 0 OR r.implementationsCount IS NULL")
    ...
    @Query("SELECT r FROM RequestEntityCodeBase r LEFT JOIN FETCH r.implementations WHERE r.id = :id")
    Optional<RequestEntity> findByIdWithImplementations(@Param("id") String id);

    @Query("SELECT r FROM RequestEntityCodeBase r LEFT JOIN FETCH r.tags LEFT JOIN FETCH r.implementations WHERE r.id = :id")
    Optional<RequestEntity> findByIdWithAllDetails(@Param("id") String id);

    @Query("SELECT DISTINCT r FROM RequestEntityCodeBase r JOIN r.implementations i WHERE i.language = :language")
    List<RequestEntity> findByImplementationLanguage(@Param("language") String language);

    @Query("SELECT r.method, COUNT(r) FROM RequestEntityCodeBase r GROUP BY r.method")
    List<Object[]> getRequestStatsByMethod();

    @Query("SELECT c.name, COUNT(r) FROM RequestEntityCodeBase r JOIN r.collection c GROUP BY c.id, c.name")
    List<Object[]> getRequestStatsByCollection();

    List<RequestEntity> findByCreatedAtBetween(LocalDateTime start, LocalDateTime end);

    @Query("SELECT r FROM RequestEntityCodeBase r WHERE r.implementationsCount > 0 ORDER BY r.updatedAt DESC")
    List<RequestEntity> findMostRecentlyUsed(Pageable pageable);

    @Query("SELECT r FROM RequestEntityCodeBase r ORDER BY r.implementationsCount DESC")
    ...
    @Modifying
    @Transactional
    @Query("UPDATE RequestEntityCodeBase r SET r.isFavorite = :isFavorite WHERE r.id IN :ids")
