...
import com.usg.autoAPIGenerator.entities.postgres.codeBase.HeaderEntity;
...
@Repository("HeaderRepositoryCodeBase")
public interface HeaderRepository extends JpaRepository<HeaderEntity, String> {
    ...
    @Query("SELECT h FROM HeaderEntityCodeBase h WHERE h.requestEntity.id = :requestId")
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM HeaderEntityCodeBase h WHERE h.requestEntity.id = :requestId")
    void deleteByRequestId(@Param("requestId") String requestId);

    // Additional useful methods you might want:

    @Query("SELECT h FROM HeaderEntityCodeBase h WHERE h.requestEntity.id = :requestId AND h.key = :key")
    Optional<HeaderEntity> findByRequestIdAndKey(@Param("requestId") String requestId, @Param("key") String key);

    @Query("SELECT h FROM HeaderEntityCodeBase h WHERE h.requestEntity.id = :requestId AND h.required = true")
    List<HeaderEntity> findRequiredByRequestId(@Param("requestId") String requestId);

    @Query("SELECT h FROM HeaderEntityCodeBase h WHERE h.requestEntity.id = :requestId ORDER BY h.key ASC")
    List<HeaderEntity> findByRequestIdOrdered(@Param("requestId") String requestId);

    @Modifying
    @Transactional
    @Query("DELETE FROM HeaderEntityCodeBase h WHERE h.requestEntity.id = :requestId AND h.key = :key")
    void deleteByRequestIdAndKey(@Param("requestId") String requestId, @Param("key") String key);

    @Query("SELECT COUNT(h) FROM HeaderEntityCodeBase h WHERE h.requestEntity.id = :requestId")
