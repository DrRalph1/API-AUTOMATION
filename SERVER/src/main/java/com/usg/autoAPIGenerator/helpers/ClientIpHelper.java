...
public class ClientIpHelper {
    ...
    public boolean validateSwaggerClientIp(String apiKey, String apiSecret) {
        ...
        if (apiKey == null || apiKey.isBlank() || apiSecret == null || apiSecret.isBlank()) {
            loggerUtil.log("auto-api-generator", "[Swagger Client IP Validation] ❌ Missing API key or secret.");
        ...
        try {
            ...
            serverIp = "unknown";
            loggerUtil.log("auto-api-generator", "[Swagger Client IP Validation] ⚠ Unable to resolve server IP: " + ex.getMessage());
        ...
        try {
            ...
            if (isValid) {
                loggerUtil.log("auto-api-generator", "[Swagger Client IP Validation] ✅ Client IP validated successfully | apiKey=" + apiKey + " | IP=" + serverIp);
                ...
                loggerUtil.log("auto-api-generator", "[Swagger Client IP Validation] ❌ Invalid client IP | apiKey=" + apiKey + " | IP=" + serverIp);
            ...
            loggerUtil.log("auto-api-generator", "[Swagger Client IP Validation] 🚨 Error validating client IP | apiKey=" + apiKey +
...
//                    " | IP=" + serverIp + " | error=" + e.getMessage());
    ...
    public boolean validateAPIClientIp(String apiKey, String apiSecret) {
        ...
        // ---------------------------------
        if (apiKey == null || apiKey.isBlank() || apiSecret == null || apiSecret.isBlank()) {
            loggerUtil.log("auto-api-generator", "[API Client IP Validation] ❌ Missing API key or secret.");
        ...
        try {
            ...
            serverIp = "unknown";
            loggerUtil.log("auto-api-generator", "[API Client IP Validation] ⚠ Unable to resolve server IP: " + ex.getMessage());
        ...
        try {
            // Query API key & secret + server IP
            String query = "SELECT COUNT(*) FROM tb_sys_api_validation " +
                    "WHERE api_key = ? AND api_secret = ? AND server_ip = ? AND micro_service_id = 'auto-api-generator'";
            ...
            if (isValid) {
                loggerUtil.log("auto-api-generator", "[API Client IP Validation] ✅ Client IP validated successfully | apiKey=" + apiKey + " | IP=" + serverIp);
                // System.out.println("[API Client IP Validation] ✅ Client IP validated successfully | apiKey=" + apiKey + " | IP=" + serverIp);
            } else {
                loggerUtil.log("auto-api-generator", "[API Client IP Validation] ❌ Invalid client IP | apiKey=" + apiKey + " | IP=" + serverIp);
            ...
            loggerUtil.log("auto-api-generator", "[API Client IP Validation] 🚨 Error validating client IP | apiKey=" + apiKey +
                    " | IP=" + serverIp + " | error=" + e.getMessage());
