...
public class ApiKeyNSecretHelper {
    ...
    public boolean validateSwaggerCredentials(String apiKey, String apiSecret) {
        ...
        if (apiKey == null || apiKey.isBlank() || apiSecret == null || apiSecret.isBlank()) {
            loggerUtil.log("auto-api-generator", "[Swagger Validation] ❌ Missing API key or secret.");
        ...
        try {
            ...
            if (isValid) {
                loggerUtil.log("auto-api-generator", "[Swagger Validation] ✅ API key & secret validated successfully | apiKey=" + apiKey);
                // System.out.println("[Swagger Validation] ✅ API key & secret validated successfully | apiKey=" + apiKey);
            } else {
                loggerUtil.log("auto-api-generator", "[Swagger Validation] ❌ Invalid API key or secret | apiKey=" + apiKey);
            ...
            loggerUtil.log("auto-api-generator", "[Swagger Validation] 🚨 Error during validation | apiKey=" + apiKey +
                    " | error=" + e.getMessage());
...
//                    " | error=" + e.getMessage());
    ...
    public boolean validateApiCredentials(String apiKey, String apiSecret) {
        ...
        // ---------------------------------
        if (apiKey == null || apiKey.isBlank() || apiSecret == null || apiSecret.isBlank()) {
            loggerUtil.log("auto-api-generator", "[API Validation] ❌ Missing API key or secret.");
            // System.out.println("[API Validation] ❌ Missing API key or secret.");
            return false;
        }

        try {
            // Query API key & secret for auto-api-generator microservice
            String query = "SELECT COUNT(*) FROM tb_sys_api_validation " +
                    "WHERE api_key = ? AND api_secret = ? AND micro_service_id = 'auto-api-generator'";
            ...
            if (isValid) {
                loggerUtil.log("auto-api-generator", "[API Validation] ✅ API key & secret validated successfully | apiKey=" + apiKey);
                // System.out.println("[API Validation] ✅ API key & secret validated successfully | apiKey=" + apiKey);
            } else {
                loggerUtil.log("auto-api-generator", "[API Validation] ❌ Invalid API key or secret | apiKey=" + apiKey);
            ...
            loggerUtil.log("auto-api-generator", "[API Validation] 🚨 Error during validation | apiKey=" + apiKey +
                    " | error=" + e.getMessage());
