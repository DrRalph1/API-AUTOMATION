...
public class AppUserService {
    ...
    // List of valid sort fields for AppUserEntity
    ...
    // ========== CREATE USER ==========
    @Transactional
    public ResponseEntity<?> createUser(AppUserDTO dto, String requestId, HttpServletRequest req, String performedBy) {
        ...
        try {
            ...
            Optional<AppUserEntity> existingUser = appUserRepository.findByUsername(dto.getUsername());
            ...
            AppRoleEntity role = roleRepository.findById(dto.getRoleId())
    ...
    public ResponseEntity<?> userLogin(UserLoginRequestDTO userLoginRequestDTO, String requestId,
        ...
        try {
            ...
            Optional<AppUserEntity> userOptional = appUserRepository.findByUserIdIgnoreCase(userId);
    ...
    public ResponseEntity<?> resetPassword(
        ...
        try {
            ...
            Optional<AppUserDTO> userOptional = appUserRepository.getUserByUserId(userId);
            if (userOptional.isEmpty()) {
                loggerUtil.log("web-application-firewall",
...
//            }
    ...
    public ResponseEntity<?> resetDefaultPassword(ResetPasswordRequestDTO resetRequest, String requestId,
        ...
        try {
            ...
            Optional<AppUserDTO> updatedUserOptional = appUserRepository.getUserByUserId(userId);
    ...
    // ========== BULK CREATE USERS ==========
    @Transactional
    public ResponseEntity<?> createUsersBulk(List<AppUserDTO> dtos, String requestId, HttpServletRequest req, String performedBy) {
        if (requestId == null || requestId.isEmpty()) {
            requestId = UUID.randomUUID().toString();
        }
        ...
        List<AppUserEntity> entitiesToSave = new ArrayList<>();
        int createdCount = 0;
        ...
        if (!entitiesToSave.isEmpty()) {
            try {
                List<AppUserEntity> savedEntities = appUserRepository.saveAll(entitiesToSave);

                // Update results with created user IDs
                int savedIndex = 0;
                for (UserResultDTO result : results) {
                    if ("PENDING_CREATION".equals(result.getStatus())) {
                        AppUserEntity savedEntity = savedEntities.get(savedIndex);
    ...
    public Map<String, Object> getAllUsers(Pageable pageable, String requestId, HttpServletRequest req, String performedBy) {
        ...
        // Get paginated users
        Page<AppUserEntity> usersPage = appUserRepository.findAll(pageable);

        // Map entities to DTOs - you already do this mapping elsewhere; adapt if needed
        List<AppUserDTO> users = usersPage.stream().map(entity -> AppUserDTO.builder()
                        .userId(entity.getUserId())
                        .username(entity.getUsername())
                        .fullName(entity.getFullName())
    ...
    public ResponseEntity<?> searchUsers(
        ...
        try {
            ...
            Specification<AppUserEntity> spec = (root, query, criteriaBuilder) -> {
            ...
            Page<AppUserEntity> pageResult = appUserRepository.findAll(spec, fixedPageable);

            Page<AppUserDTO> page = pageResult.map(entity -> AppUserDTO.builder()
                    .userId(entity.getUserId())
                    .username(entity.getUsername())
                    .fullName(entity.getFullName())
                    .roleId(entity.getRole().getRoleId())
    ...
    // ========== UPDATE USER ==========
    @Transactional
    public ResponseEntity<?> updateUser(String userId, AppUserDTO dto, String requestId, HttpServletRequest req, String performedBy) {
        Map<String, Object> result = new HashMap<>();
        ...
        try {
            ...
            if (dto.getEmailAddress() != null && !dto.getEmailAddress().equals(entity.getEmailAddress())) {
                Optional<AppUserEntity> existingEmail = appUserRepository.findByEmailAddress(dto.getEmailAddress());
            ...
            if (!entity.getRole().getRoleId().equals(dto.getRoleId())) {
                AppRoleEntity newRole = roleRepository.findById(dto.getRoleId())
                        .orElseThrow(() -> new ResourceNotFoundException(
                                String.format("Role with ID %s not found", dto.getRoleId())
                        ));
    ...
    private void validateUserUpdateBusinessRules(AppUserDTO dto) {
        // Validate username
    ...
    private void validateUserBusinessRules(AppUserDTO dto) {
        // Validate username
        if (dto.getUsername() == null || dto.getUsername().trim().isEmpty()) {
            throw new BusinessRuleException("Username cannot be empty");
        }
    ...
    private boolean checkIfUserIsInUse(String userId) {
        ...
        // You might want to check if user has any created records in the system
