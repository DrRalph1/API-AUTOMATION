...
public class PostgreSQLApiExecutionHelper {
    ...
    public GeneratedApiEntity createAndSaveApiEntity(
            ...
            PostgreSQLParameterGeneratorUtil parameterGenerator,
    ...
    public ExecuteApiRequestDTO prepareValidatedRequest(GeneratedApiEntity api, ExecuteApiRequestDTO executeRequest) {
        ...
        if (("POST".equals(api.getHttpMethod()) || "PUT".equals(api.getHttpMethod()))
            ...
            if (!hasContentType) {
                ...
                log.debug("Auto-added Content-Type: application/json for {} request",
                        api.getHttpMethod());
            }
        }
    ...
    public Object executeAgainstPostgreSQL(
            ...
            PostgreSQLTableExecutorUtil tableExecutorUtil,
            PostgreSQLViewExecutorUtil viewExecutorUtil,
            PostgreSQLProcedureExecutorUtil procedureExecutorUtil,
            PostgreSQLFunctionExecutorUtil functionExecutorUtil,
        ...
        String targetSchema = sourceObject.getTargetOwner() != null ?
                sourceObject.getTargetOwner().trim().toLowerCase() :
                (sourceObject.getOwner() != null ? sourceObject.getOwner().trim().toLowerCase() : null);

        log.info("Executing PostgreSQL operation - Type: {}, Name: {}, Schema: {}", targetType, targetName, targetSchema);
        ...
        // Create consolidated params map
        ...
        switch (targetType) {
            case "TABLE":
                return executeTableOperation(tableExecutorUtil, targetName, targetSchema, operation,
                        consolidatedParams, api, configuredParamDTOs);
            case "VIEW":
            case "MATERIALIZED VIEW":
                String viewSchema = targetSchema;
                if (viewSchema == null || viewSchema.isEmpty()) {
                    viewSchema = sourceObject.getOwner();
                    if (viewSchema == null || viewSchema.isEmpty()) {
                        viewSchema = sourceObject.getSchemaName();
                    }
                }
                return viewExecutorUtil.execute(api, sourceObject, targetName, viewSchema, request,
                        configuredParamDTOs);
            case "PROCEDURE":
                return procedureExecutorUtil.execute(api, sourceObject, targetName, targetSchema, request,
                        configuredParamDTOs);
            case "FUNCTION":
                return functionExecutorUtil.execute(api, sourceObject, targetName, targetSchema, request,
                        configuredParamDTOs);
            default:
    ...
    private Object executeTableOperation(PostgreSQLTableExecutorUtil tableExecutorUtil,
                                         String tableName,
                                         String schema,
                                         String operation,
        ...
        log.info("Executing {} operation on {}.{} with params: {}", operation, schema, tableName, params);

        switch (operation.toUpperCase()) {
            case "SELECT":
                return tableExecutorUtil.executeSelect(tableName, schema, params, api, configuredParamDTOs);
            case "INSERT":
                return tableExecutorUtil.executeInsert(tableName, schema, params, api, configuredParamDTOs);
            case "UPDATE":
                return tableExecutorUtil.executeUpdate(tableName, schema, params, api, configuredParamDTOs);
            case "DELETE":
                return tableExecutorUtil.executeDelete(tableName, schema, params, api, configuredParamDTOs);
            default:
