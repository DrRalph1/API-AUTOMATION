  ...
  oracle: {
    ...
    getDDLQuery: (objectType, objectName, schema) => {
      switch(objectType?.toUpperCase()) {
        case 'TABLE':
          return `
            SELECT DBMS_METADATA.GET_DDL('TABLE', '${objectName}', '${schema || 'CBXDMX'}') AS ddl FROM DUAL;
          `;
        case 'VIEW':
          return `
            SELECT DBMS_METADATA.GET_DDL('VIEW', '${objectName}', '${schema || 'CBXDMX'}') AS ddl FROM DUAL;
          `;
        case 'FUNCTION':
        case 'PROCEDURE':
          return `
            SELECT DBMS_METADATA.GET_DDL('${objectType.toUpperCase()}', '${objectName}', '${schema || 'CBXDMX'}') AS ddl FROM DUAL;
          `;
        case 'PACKAGE':
          return `
            SELECT DBMS_METADATA.GET_DDL('PACKAGE', '${objectName}', '${schema || 'CBXDMX'}') AS ddl FROM DUAL;
          `;
        case 'TRIGGER':
          return `
            SELECT DBMS_METADATA.GET_DDL('TRIGGER', '${objectName}', '${schema || 'CBXDMX'}') AS ddl FROM DUAL;
          `;
        default:
...
`
  ...
  oracle: {
    TABLE: (name, schema) => `-- DDL for table ${schema || 'CBXDMX'}.${name}
CREATE TABLE ${schema || 'CBXDMX'}.${name} (
...
-- Add your columns here:
-- ALTER TABLE ${schema || 'CBXDMX'}.${name} ADD (column_name VARCHAR2(255));
`,
    VIEW: (name, schema) => `-- View definition for ${schema || 'CBXDMX'}.${name}
CREATE OR REPLACE VIEW ${schema || 'CBXDMX'}.${name} AS
SELECT 
  id,
  created_at,
  updated_at
...
WHERE condition = 1;
`,
    FUNCTION: (name, schema) => `-- Function definition for ${schema || 'CBXDMX'}.${name}
CREATE OR REPLACE FUNCTION ${schema || 'CBXDMX'}.${name}(
  p_param1 IN NUMBER
) RETURN VARCHAR2
IS
...
/
`,
    PROCEDURE: (name, schema) => `-- Procedure definition for ${schema || 'CBXDMX'}.${name}
CREATE OR REPLACE PROCEDURE ${schema || 'CBXDMX'}.${name}(
  p_param1 IN NUMBER,
...
/
`,
    PACKAGE: (name, schema) => `-- Package specification for ${schema || 'CBXDMX'}.${name}
CREATE OR REPLACE PACKAGE ${schema || 'CBXDMX'}.${name} IS
  -- Public constants
...
-- Package body
CREATE OR REPLACE PACKAGE BODY ${schema || 'CBXDMX'}.${name} IS
  PROCEDURE init IS
  BEGIN
...
/
`,
    TRIGGER: (name, schema) => `-- Trigger definition for ${schema || 'CBXDMX'}.${name}
CREATE OR REPLACE TRIGGER ${schema || 'CBXDMX'}.${name}
  BEFORE UPDATE ON your_table
  FOR EACH ROW
BEGIN
