 ...
 * @param {Object} roleData - UserRole DTO
 ...
 * @param {string} authorizationHeader - Bearer token
 * @param {Array} rolesData - Array of UserRole DTOs
 * @returns {Promise} API response
 */
 ...
 * @returns {Object} UserRole DTO
 */
export const buildUserRoleDTO = (roleData) => {
    const {
        roleCode,
        roleName,
        description = '',
 ...
 * @returns {Array} Array of UserRole DTOs
 */
export const buildBulkRolesRequest = (roles) => {
    ...
    return roles.map(role => buildUserRoleDTO(role));
};
